	...
	internal class AddressDeleteCommandHandler : IRequestHandler<AddressDeleteCommand, Result<int>>
	{
		...
		public async Task<Result<int>> Handle(AddressDeleteCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				entity = _mapper.Map<AddressEntity>(command);
				entity.Status =(int)StatusEnum.Delete;
				await _unitOfWork.Repository<AddressEntity>().UpdateFieldsAsync(entity,
					x => x.Status);
				var result = await _unitOfWork.Save(cancellationToken);
				return result > 0
						? await Result<int>.SuccessAsync(entity.AddressId, "Cập nhật dữ liệu thành công.")
