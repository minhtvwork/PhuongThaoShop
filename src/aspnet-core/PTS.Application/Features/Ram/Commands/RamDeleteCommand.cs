...
{
	public record RamDeleteCommand : IRequest<Result<int>>, IMapFrom<RamEntity>
	{
		public int Id { get; set; }
	}

	internal class RamDeleteCommandHandler : IRequestHandler<RamDeleteCommand, Result<int>>
	{
		private readonly IUnitOfWork _unitOfWork;
		private readonly IMapper _mapper;
		public RamDeleteCommandHandler(IUnitOfWork unitOfWork, IMapper mapper)
		{
			_unitOfWork = unitOfWork;
			_mapper = mapper;
		}
		public async Task<Result<int>> Handle(RamDeleteCommand command, CancellationToken cancellationToken)
		{
			try
			{
				...
				await _unitOfWork.Repository<RamEntity>().UpdateFieldsAsync(entity,
					x => x.Status);
