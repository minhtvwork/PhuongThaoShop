...
{
    public record ProductTypeCreateOrUpdateCommand : IRequest<Result<int>>, IMapFrom<ProductTypeEntity>
    {
    ...
    internal class ProductTypeCreateOrUpdateCommandHandler : IRequestHandler<ProductTypeCreateOrUpdateCommand, Result<int>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;

        public ProductTypeCreateOrUpdateCommandHandler(IUnitOfWork unitOfWork, IMapper mapper)
        {
            _unitOfWork = unitOfWork;
            _mapper = mapper;
        }

        public async Task<Result<int>> Handle(ProductTypeCreateOrUpdateCommand command, CancellationToken cancellationToken)
        {
            try
            {
                command.Status = 1;
                if (command.Id > 0)
                {
                    ...
                    {
                        var existing = await _unitOfWork.Repository<ProductTypeEntity>().Entities.AsNoTracking()
                    ...
                    await _unitOfWork.Repository<ProductTypeEntity>().UpdateAsync(entity.Id, entity);
                }
                else
                {
                    // Create logic
                    ...
                    var entity = _mapper.Map<ProductTypeEntity>(command);
                    entity.CrDateTime = DateTime.Now;
                    await _unitOfWork.Repository<ProductTypeEntity>().AddAsync(entity);
                }
