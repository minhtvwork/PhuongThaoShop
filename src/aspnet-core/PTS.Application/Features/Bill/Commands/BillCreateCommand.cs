...
{
    public record BillCreateCommand : IRequest<Result<int>>, IMapFrom<BillEntity>
    {
    ...
    internal class BillCreateCommandHandler : IRequestHandler<BillCreateCommand, Result<int>>
    {
		private readonly IUnitOfWork _unitOfWork;
		private readonly IMapper _mapper;
		public BillCreateCommandHandler(IUnitOfWork unitOfWork, IMapper mapper)
		{
			_unitOfWork = unitOfWork;
			_mapper = mapper;
		}
		public async Task<Result<int>> Handle(BillCreateCommand command, CancellationToken cancellationToken)
        {
            try
            {
                //var existing = await _unitOfWork.Repository<BillEntity>().Entities.AsNoTracking()
