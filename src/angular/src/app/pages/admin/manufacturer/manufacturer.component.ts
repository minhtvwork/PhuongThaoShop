...
import { ManufacturerDto } from 'src/app/shared/models/model';
...
export class ManufacturerComponent implements OnInit {
  ...
  loadData(): void {
    this.adminService.getPageManufacturer(1, 99999, this.searchKeyword).subscribe(response => {
  ...
  save(): void {
    if (this.fbForm.valid) {
      ...
      this.isSave = true;
      this.adminService.createOrUpdateManufacturer(obj.id, obj.name).subscribe(
        (response: any) => {
  ...
  edit(item: ManufacturerDto): void {
    this.modalTitle = 'Sửa ';
  ...
  delete(id: number): void {
    this.adminService.deleteManufacturer(id).subscribe(
      (response: any) => {
